...
var lib = {
  ...
  renderGraph: function(graph, displayEdgelabels, subgraph=false) {
    const filename = "graph.out";
    var stream = fs.createWriteStream(filename);
    stream.once('open', function(fd) {
      lib.writeGraph(graph, subgraph, stream, displayEdgelabels)
      stream.end();
    });
  },

  writeGraph: function(graph, subgraph, stream, displayEdgelabels) {
    ...
    graph.subgraphs.forEach(function(s) {
      stream.write('\n')
      lib.writeGraph(s, true, stream)
    });
    graph.edges.forEach(function(e) {
      var label = "";
      if (displayEdgelabels && e.label !== undefined) {
        label = ` [label=${e.label}]`
      }
